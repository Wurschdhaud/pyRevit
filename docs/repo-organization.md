...
- `bin` is **not tracked in git**. It holds runtime binaries (DLLs, engines, CLI, CPython under `cengines/`, etc.) produced locally by `dotnet run -- ci` in [`build/`](../build/) or downloaded by `pyrevit clone` / `pyrevit clones update` from public CI Release assets. Static sources live under [`release/bin-assets/`](../release/bin-assets/), [`release/cengines/`](../release/cengines/), and [`release/pyrevit-hosts.json`](../release/pyrevit-hosts.json).
